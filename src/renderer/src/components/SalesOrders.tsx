...
import { collection, onSnapshot, query, where, orderBy, getDocs, deleteDoc, doc, writeBatch, updateDoc } from 'firebase/firestore'
...
import { BulkSalesOrderModal } from './BulkSalesOrderModal'
...
export const SalesOrders: React.FC = () => {
    ...
    const onGridReady = useCallback(async (params: any) => {
        ...
        if (user) {
            try {
                const savedState = await getUserLayout(user.uid, 'sales-orders');
                if (savedState) {
    ...
    const gridStateHandlers = useGridState('sales-orders', gridApi)
    ...
    const [deleteConfirmation, setDeleteConfirmation] = useState<{ isOpen: boolean, soId: string | null }>({
        isOpen: false,
        soId: null
    })
    ...
    // 2. Fetch Sales Orders for Selected Sheet (By Date Range)
    useEffect(() => {
        if (!selectedSheet) {
            setRowData([]);
            return;
        ...
        const year = selectedSheet.year;
        const month = selectedSheet.month;
                ...
                if (so.items && Array.isArray(so.items)) {
                    so.items.forEach((item: any) => {
                        const row: any = {
                            ...item,
                            parent_id: so.id,
                            id: so.id + "_" + item.product_id, // synthetic ID
                            date: so.date,
                            customer_name: so.customer_name,
                            po_no: so.po_no,
                            created_at: so.createdAt,
                            original_so: so // Pass full object for editing
                        };
                        flatRows.push(row);
                    });
                }
    ...
    const handleDelete = async () => {
        if (!deleteConfirmation.soId) return;

        try {
            const batch = writeBatch(db);

            // 1. Delete Sales Order
            const soRef = doc(db, 'fg_sales_orders', deleteConfirmation.soId);
            batch.delete(soRef);

            // 2. Delete Inventory Transactions
            const transQuery = query(collection(db, 'fg_inventory_transactions'), where('so_id', '==', deleteConfirmation.soId));
            ...
            await batch.commit();
            setDeleteConfirmation({ isOpen: false, soId: null });
    ...
    const columnDefs = [
        ...
        { headerName: 'UOM', field: 'uom', sortable: true, filter: true, width: 80 },
        { headerName: 'Quantity', field: 'quantity', sortable: true, filter: true, type: 'numericColumn', width: 100, valueFormatter: formatNumber },
        { headerName: 'Tolerance', field: 'tolerance', sortable: true, filter: true, type: 'numericColumn', width: 100, valueFormatter: formatNumber },
        {
            headerName: 'Actions',
            ...
            cellRenderer: (params: any) => canEdit ? (
                <div className="flex gap-2 justify-center">
                    <button onClick={() => {
                        setEditingData(params.data.original_so);
                        setShowAddModal(true);
                    }} className="p-1 hover:bg-gray-200 rounded">
                        <img src={editIcon} alt="Edit" className="w-4 h-4 opacity-70 hover:opacity-100" />
                    </button>
                    <button onClick={() => setDeleteConfirmation({ isOpen: true, soId: params.data.parent_id })} className="p-1 hover:bg-red-100 rounded">
        ...
        <div className="flex h-[calc(100vh-100px)] bg-gray-100 overflow-hidden text-gray-800 relative">
            ...
            <div className="w-64 bg-white border-r flex flex-col flex-shrink-0 shadow-sm h-full">
                <div className="p-4 border-b bg-white">
                    <h2 className="text-lg font-bold text-gray-800">Sales Order Sheets</h2>
                </div>
                ...
                <div className="flex justify-between items-center p-4 border-b">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">
                            {selectedSheet ? `Sales Orders: ${MONTHS[selectedSheet.month - 1]} ${selectedSheet.year}` : 'Sales Orders'}
                        </h1>
                    </div>
                    <div className="flex gap-3">
                        ...
                        <div className="relative">
                            ...
                            {showColManager && gridApi && (
                                <ColumnManager api={gridApi} onClose={() => setShowColManager(false)} gridId="sales-orders" />
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 p-4 flex flex-col overflow-hidden">
                    {selectedSheet ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-gray-400">
                            ...
                            <p className="text-lg">Select a sheet to view sales orders</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <ConfirmationModal
                ...
                onCancel={() => setDeleteConfirmation({ isOpen: false, soId: null })}
                onConfirm={handleDelete}
                title="Delete Sales Order"
                message="Are you sure you want to delete this sales order? This will also remove all associated inventory entries."
                isDangerous={true}
            />
        </div>
    )
